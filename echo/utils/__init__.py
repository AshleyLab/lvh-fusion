...
from . import video
from . import video_exp
from . import video_dev
...
def loadvideo_downsample(filename: str, f:int) -> np.ndarray:
    """Loads a video from a file.

    Args:
        filename (str): filename of video
        f (int): int of downsampled data
    ...
    for count in range(frame_count):
        ...
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        resized_image_ds = cv2.resize(frame, (f, f), interpolation = cv2.INTER_CUBIC) 
        resized_image_us = cv2.resize(resized_image_ds, (112, 112), interpolation = cv2.INTER_CUBIC)
        v[count] = resized_image_us
...
def loadvideo_downsample_wes(filename: str, f:int) -> np.ndarray:
    """Loads a video from a file.
    ...
    v = np.zeros((frame_count, f, f, 3), np.uint8)
...
__all__ = ["video", "segmentation", "loadvideo", "savevideo", "get_mean_and_std", "bootstrap", "latexify", "dice_similarity_coefficient", "video_gradcam"]