...
def run(
    ...
    ## Setting nummber of lead for ablation experiment
    num_leads = 12
    if remove_redundant:
        num_leads = 8
    ...
    if modelname in ['resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet152', 'resnext50_32x4d', 'resnext101_32x8d', 'wide_resnet50_2', 'wide_resnet101_2']:
        model = eval('hha.models.resnet_1D.' + modelname + '(input_channels=' + str(num_leads) +',num_classes=' + str(nodes) +')')
    ### DenseNets   
    ...
    ### Custom 
    if modelname in ['c18', 'c34']:
        model = eval('hha.models.custom_1D.' + modelname + '(num_classes=' + str(nodes) +')')
    ...
    mean,std = hha.utils.get_mean_and_std(hha.datasets.EcgAblation(split="train", 
                                        file_list=file_list, 
                                        baseline=baseline, 
                                        notch=notch, 
    ...
    kwargs = {"target_type": tasks,
              ...
              "mask_lead":mask_lead,
              "mask_lead_num": mask_lead_num,
              "mask_redundant":mask_redundant,
              "mask_redundant_random":mask_redundant_random,
              "seed":seed
              }

    train_dataset = hha.datasets.EcgAblation(split="train", **kwargs)
    train_dataloader = torch.utils.data.DataLoader(train_dataset
                                                    ...
                                                    , drop_last=True)
    val_dataloader = torch.utils.data.DataLoader(hha.datasets.EcgAblation(split="validate", **kwargs)
                                                , batch_size=batch_size
                                                , num_workers=num_workers
                                                , shuffle=True
                                                , pin_memory=(device.type == "cuda"))
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        ...
        if run_test:
            for split in ["validate", "test"]:
                ...
                dataloader = torch.utils.data.DataLoader(hha.datasets.EcgAblation(split=split, **kwargs)
                                                        ,batch_size=batch_size
                                                        , num_workers=num_workers
                                                        , shuffle=True
                                                        , pin_memory=(device.type == "cuda"))
